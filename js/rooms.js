...
export async function initRooms({ dotsContainer, onRoomChange, onPanelReset }) {
  ...
  // Cross-device sync: when the user tabs back in, re-pull rooms from
  // the server so edits made elsewhere show up. Skipped when we have
  // pending local edits so nothing gets stomped.
